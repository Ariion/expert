...
export async function GET(req: Request) {
  ...
  if (!userId) return redirectTo("/login", { error: "invalid_token" });
  ...
  try {
    const [user] = await sql<{ email: string; onboarding_sent_at: Date | null; service_name: string | null }[]>`
      select u.email, u.onboarding_sent_at, s.name as service_name
        from users u
        left join services s on s.id = u.signup_service_id
       where u.id = ${userId}
    `;
    if (user && !user.onboarding_sent_at) {
      const tpl = welcomeEmail(user.service_name ?? undefined);
